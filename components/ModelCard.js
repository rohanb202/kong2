...
export default function ModelCard({ModelData}) {
  ...
  return (
    <Link href={`/${ModelData.author}/${ModelData._id}`}>
        <div className={` p-2 group hover:cursor-pointer mr-2 mt-2 rounded-md backdrop-blur-sm  ${theme=='dark'?" bg-gradient-to-l from-slate-300 to-slate-100 border-[1px] text-black ":" bg-gradient-to-l from-slate-700 to-slate-900 text-white "}   w-[20rem]     md:w-[25rem] flex flex-grow flex-col `}>
            <div>
                <h2 className='group-hover:text-blue-400 dark:group-hover:text-blue-600'>{ModelData.author}/{ModelData.title}</h2>            
            </div>
