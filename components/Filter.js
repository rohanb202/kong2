...
export default function Filter() {
  ...
  function tagHandler(e) {
    ...
    router.push(
      pathname + "?" + createQueryString("tag", e.target.textContent)
    );
  }
    ...
    <div className="flex flex-col flex-wrap ">
      <div className="flex flex-wrap justify-between gap-1 p-4 text-xs ">
        <button
          alt="tasks"
          className={`btnFilter ${
            !currentFilter || "tasks" === currentFilter
              ? "text-white"
              : "text-white/50"
          }`}
          onClick={() => {
        ...
        <button
          alt="libraries"
          className={`btnFilter ${
            !currentFilter || "libraries" === currentFilter
              ? "text-white"
              : "text-white/50"
          }`}
          onClick={() => {
            setCurrentFilter("libraries");
          }}
        >
          Libraries
        </button>
        <button
          alt="datasets"
          className={`btnFilter ${
            !currentFilter || "datasets" === currentFilter
              ? "text-white"
              : "text-white/50"
          }`}
          onClick={() => {
        ...
        <button
          alt="languages"
          className={`btnFilter ${
            !currentFilter || "languages" === currentFilter
              ? "text-white"
              : "text-white/50"
          }`}
          onClick={() => {
            setCurrentFilter("languages");
          }}
        >
          Languages
        </button>
      ...
      <div className="flex items-center">
        <Input
          ...
          className=" bg-slate-100 dark:bg-[rgb(18,18,18)]"
          type="text"
          placeholder="Filter"
        />
        {select && (
          <button
            ...
            className="w-20 p-1 m-2 text-xs text-white dark:bg-[rgb(18,18,18)] dark:border-[1px] bg-gray-800 rounded-md text-nowrap"
          >
            Reset Filter
          </button>
        )}
      ...
      <div className="flex flex-wrap justify-start p-4">
        {!loading &&
          filteredSearch?.map((category) => (
            <button
              alt="select tag"
              className={`p-2 dark:bg-black dark:border-[1px] m-1 text-xs ${
                !select || select === category.title
                  ? "text-white"
                  : "text-white/50"
              } bg-gray-800 rounded-md backdrop-blur-sm `}
              key={category._id}
              onClick={tagHandler}
            >
              {category.title}
