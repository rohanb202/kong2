    ...
    if (method === "POST") {
      ...
      if(method === "PUT") {
        ...
        try{
          ...
          const model = await db.collection('models').findOne({ _id: objectIdModelId });

          // Check if the author of the model matches the current user
          // console.log(model, user.userID);
          if (model && model.author === user.userID) { // Assuming user.userId is the ID of the current user
            ...
            return res.status(200).json({ message: 'Markdown updated successfully' });
          }
          else{
